...
def ath_threshold(f):
    f = np.maximum(f, 0.01)
    f_khz = f / 1000
    ath = 3.64 * (f_khz**-0.8) - 6.5 * np.exp(-0.6 * (f_khz - 3.3)**2) + 10**-3 * (f_khz**4)
    return ath - 60
...
def compute_masking_threshold(magnitudes, freqs, threshold_offset):
    ...
    peaks, _ = scipy_signal_find_peaks(mag_db, height=-80)
    ...
    for p in peaks:
        ...
        spread = np.where(delta_bark < 0, 15 * delta_bark, -10 * delta_bark)
        
        masking_curve = np.maximum(masking_curve, target_mag + spread - 15)

    global_threshold_db = np.maximum(ath, masking_curve) + threshold_offset
    
    return np.clip(global_threshold_db, -100, None)